...
> * push가 보내지면 메시지와함께 `네이버me`의 `알림`에 알림이 전송되고, 모바일기기에 설치된 `네이버앱`에 push가 전달됩니다.
> * 위의 내용만으로는 `notification`를 항상 `true`로 보내는것이 좋을것같습니다. 그러나 몇몇상황에서 `notification`는 부작용을 일으킬 수 있습니다.
...
> - 텍스트에 `https://talk.naver.com/`처럼 URL이 삽입된경우 자동으로 Anchor Tag(`<a>`)로 변환하여 노출해준다.
